...
const Settings = () => {
  ...
  const handleChangePassword = async () => {
    const { value: newPassword } = await MySwal.fire({
      ...
      inputPlaceholder: 'يجب ألا تقل عن 6 أحرف',
      showCancelButton: true,
      confirmButtonText: 'تحديث',
      cancelButtonText: 'إلغاء',
      confirmButtonColor: '#10b981',
      inputValidator: (value) => {
        if (!value) return 'لا يمكنك ترك الحقل فارغاً!';
        if (value.length < 6) return 'يجب أن تتكون من 6 أحرف على الأقل!';
      }
    });

    if (newPassword) {
      MySwal.fire({ title: 'جاري التحديث...', didOpen: () => MySwal.showLoading() });
      const { error } = await supabase.auth.updateUser({ password: newPassword });
      if (error) {
        MySwal.fire({ icon: 'error', title: 'خطأ', text: error.message });
      } else {
        MySwal.fire({ icon: 'success', title: 'تم التحديث', text: 'تم تغيير كلمة المرور بنجاح!' });
      }
    }
  };
            ...
            <div className="bg-surface-container-lowest rounded-xl overflow-hidden">
              <div className="p-6 md:p-8 border-b border-surface">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <button onClick={handleChangePassword} className="px-6 py-2.5 bg-primary text-white rounded-xl font-bold text-sm w-full md:w-auto hover:bg-primary/90 transition">تغيير كلمة المرور</button>
                </div>
              </div>
